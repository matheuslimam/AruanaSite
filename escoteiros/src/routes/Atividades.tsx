...
export default function Atividades() {
  ...
  const [bonusOpen, setBonusOpen] = useState(false) // ⬅️ inicia recolhido
  ...
  const sortActivities = (arr: ActivityRow[]) =>
    [...arr].sort((a, b) => (startMs(a) - startMs(b)) || (a.title || '').localeCompare(b.title || ''))
  ...
  async function persistDiff({ diffOnly }: { diffOnly: boolean }): Promise<boolean> {
    ...
    const presenceChanged = !diffOnly || basePoints !== snapBasePoints || !eq(present, snapPresent)

    if (presenceChanged) {
      // sempre apaga os pontos de presença se houve mudança (inclusive ao zerar)
      await supabase.from('points').delete()
        .eq('activity_id', selected.id).eq('reason', presenceReason)

      // recria só se basePoints != 0
      if (basePoints !== 0 && Object.values(present).some(Boolean)) {
    ...
    const extrasChanged =
      !diffOnly ||
      basePoints !== snapBasePoints ||
      !eq(extraDefs, snapExtraDefs) ||
      !eq(extrasSelected, snapExtrasSelected)

    if (extrasChanged) {
      // apaga por UNIÃO (atuais + antigos) para não deixar lixo ao remover um extra
      const reasonsToDelete = new Set<string>()
      for (const d of extraDefs) reasonsToDelete.add(`${d.label} em ${selected.title}`)
      for (const d of snapExtraDefs) reasonsToDelete.add(`${d.label} em ${selected.title}`)
          ...
          for (const def of extraDefs) {
            if (sel[def.key]) {
              items.push({
                ...
                points: basePoints,
                reason: `${def.label} em ${selected.title}`
              })
            }
          }
        }
    ...
    // 4) BÔNUS por patrulha (aceita negativo)
    const allPatrolIds = Array.from(new Set([
      ...Object.keys(bonusByPatrol),
      ...Object.keys(snapBonusByPatrol)
    ]))
    ...
    for (const pid of allPatrolIds) {
      ...
      if (!diffOnly || nowVal !== snapVal) {
        ...
        await supabase.from('points').delete()
          .eq('activity_id', selected.id).eq('reason', reason).eq('patrol_id', pid)

        if (nowVal !== 0) { // permite negativo também
  ...
  async function salvarAlteracoes() {
    ...
    try {
      ...
      alert(changed ? 'Alterações salvas!' : 'Nada para salvar.')
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="max-w-6xl mx-auto px-4 py-6">
      ...
      <ul className="mt-4 grid gap-3">
        {visibleActivities.map(a => {
          ...
          return (
            <li key={a.id} className={`rounded-xl border transition hover:shadow-sm ${isOpen ? 'ring-1 ring-slate-200 bg-slate-50' : ''}`}>
              {/* Cabeçalho do item */}
              <button
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <svg width="16" height="16" viewBox="0 0 24 24" className={`transition ${isOpen ? 'rotate-180' : ''}`}><path d="M6 9l6 6 6-6" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" /></svg>
                  </div>
                </div>
              </button>

              {/* Ações do item (excluir) */}
              ...
              {/* PAINEL EXPANDIDO — detalhes + chamada/pontos */}
              {isOpen && selected && (
                <div className="px-3 pb-3 space-y-4">
                  ...
                  <div className={`rounded-2xl border p-3 ring-1 ring-inset ${selectedIsToday ? 'ring-emerald-200' : 'ring-slate-200'}`}>
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={(e) => { e.stopPropagation(); openQrModal() }} className="px-2 py-1 rounded-lg border text-sm hover:bg-gray-50">Check-in</button>
                        {!isEditingMeta ? (
                          <button onClick={(e) => { e.stopPropagation(); startEditMeta() }} className="px-2 py-1 rounded-lg border text-sm hover:bg-gray-50">Editar</button>
                        ) : (
                          <div className="flex gap-2">
                            <button onClick={(e) => { e.stopPropagation(); saveEditMeta() }} className="px-2 py-1 rounded-lg bg-black text-white text-sm">Salvar</button>
                            <button onClick={(e) => { e.stopPropagation(); cancelEditMeta() }} className="px-2 py-1 rounded-lg border text-sm">Cancelar</button>
                          </div>
                        )}
                      </div>
                    </div>
                        ...
                        return (
                          <div key={m.id} className="p-3">
                            <div className="flex items-start justify-between gap-3">
                              ...
                              <div>
                                {cat && (
                                  <span className={`text-[10px] px-1.5 py-0.5 rounded ring-1 ring-inset ${cat === 'lobinhos' ? 'bg-emerald-100 text-emerald-700 ring-emerald-300' :
                              ...
                              <tr key={m.id} className={`${stripe} border-b`}>
                                ...
                                <td className="py-2">
                                  <div className={`inline-flex items-center gap-2 px-2 py-0.5 rounded-lg ring-1 ring-inset ${ring}`}>
                                    ...
                                    {cat && (
                                      <span className={`text-[10px] px-1.5 py-0.5 rounded ${cat === 'lobinhos' ? 'bg-emerald-100 text-emerald-700' :
                  ...
                  {/* BÔNUS POR PATRULHA (colapsável) */}
                  <div className="rounded-2xl border overflow-hidden">
                    {/* header */}
                    <button
                      type="button"
                      onClick={(e) => { e.stopPropagation(); setBonusOpen(o => !o) }}
                      className="w-full flex items-center justify-between gap-2 px-3 py-2 bg-gradient-to-r from-slate-50 to-white text-left"
                      aria-expanded={bonusOpen}
                    >
                      <span className="font-medium">Bônus por Patrulha</span>
                      <svg
                        width="16" height="16" viewBox="0 0 24 24"
                        className={`transition ${bonusOpen ? 'rotate-180' : ''}`}
                      >
                          ...
                          <div key={cat} className="space-y-2">
                            ...
                            <div className="grid md:grid-cols-2 gap-2">
                              {patrolsByCat[cat].length === 0 ? (
                                ...
                                <div
                                  key={p.id}
                                  className="flex items-center justify-between gap-2 rounded-lg border px-3 py-2 bg-white"
                                >
                                  ...
                                  <input
                                    ...
                                    onChange={e => setBonusByPatrol(prev => ({
                                      ...prev,
...
{selected && (
  <>
    {/* (opcional) um spacer minúsculo quando a safe area existir */}
    ...
    ">
      <div className="max-w-6xl mx-auto px-4 py-2 flex items-center gap-2">
        {/* primário: você pode deixar só "Atualizar" se preferir */}
                    <button
        ...
        <div className="ml-auto text-xs md:text-sm text-slate-600">
